...
def dblf_packing_algorithm(packages, ulds):
    ...
    packages = sorted(packages, key=lambda pkg: pkg.get_volume(), reverse=True)
...
result_uld = dblf_packing_algorithm(packages, containers)
...
def output_results_unique(ulds, packages, output_file="results.txt"):
    ...
    with open(output_file, "w") as file:
        ...
        file.write(
            f"{total_packed_volume},{total_packed_weight},{total_packed_boxes}\n"
        )
...
def visualize_packing(containers):
    ...
    for idx, uld in enumerate(containers):
        ...
        for package in uld.packages:
            ...
            ax.add_collection3d(
                Poly3DCollection(
                    faces, facecolors="blue", edgecolors="black", alpha=0.7
                )
            )
