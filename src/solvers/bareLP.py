        ...
        for i in range(n):
            ...
            for j in range(n):
                if i != j:
                    ...
                    solver.Add(
                        z[j] + dj * Zdi[j] + hj * (1 - Zwi[j] - Zdi[j]) + wj * Zwi[j]
            ...
            solver.Add(Zwi[i] - Yhi[i] + Zdi[i] >= 0, f"orientation_height_ZY_lb_{i}")
            solver.Add(Zwi[i] - Yhi[i] + Zdi[i] <= 1, f"orientation_height_ZY_ub_{i}")
