...
def do_cuboids_touch(cuboid1: Package, cuboid2: Package) -> bool:
    ...
    (x1_min, y1_min, z1_min), (x1_max, y1_max, z1_max) = (
        (cuboid1.corners[0].x, cuboid1.corners[0].y, cuboid1.corners[0].z),
        (cuboid1.corners[1].x, cuboid1.corners[1].y, cuboid1.corners[1].z),
    )
    (x2_min, y2_min, z2_min), (x2_max, y2_max, z2_max) = (
        (cuboid2.corners[0].x, cuboid2.corners[0].y, cuboid2.corners[0].z),
        (cuboid2.corners[1].x, cuboid2.corners[1].y, cuboid2.corners[1].z),
    )
    ...
    touch_y = (
        ...
        and (x1_min <= x2_max and x1_max >= x2_min)
        and (z1_min <= z2_max and z1_max >= z2_min)
    )
    touch_z = (
...
def graphFamilyCost(uld: ULD, family: dict[Package, int]) -> float:
    ...
    ----------
    uld:
        uld over which the cost is calcualted

    family:
        family dictionary which maps packages to family index (which is an integer)
    ...
    # for a given family number, how many packages are for that
    familyList = invert_family_map(family)

    # pairwise distance matrix using floyed warshall algo -> O(N^3)
    pairWiseDist = floyd_warshall(adjMatrix)
        ...
        for packagesInFamily in familyList[familyNumber]:
            ...
            for i in range(len(packagesInFamily)):
                for j in range(len(packagesInFamily)):
                    distance = pairWiseDist[packageToIndex[packagesInFamily[i]]][
                        packageToIndex[packagesInFamily[j]]
                    ]

                    # when a box is not connected only, then ignore it.

                    if distance != np.inf:
                        cost += distance

            cost /= 2 * len(packagesInFamily)
            costs.append(cost)
